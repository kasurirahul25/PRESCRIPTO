...
const Login = () => {
  ...
  return (
    <form className='min-h-[80vh] flex items-center'>
      <div className='flex flex-col gap-3 m-auto items-start p-8 min-w-[340px] sm:min-w-96 border rounded-x1 text-zinc-600 text-sm shadow-1g'>
        <p className='text-2xl font-semibold'>{state == 'Sign Up' ? "Create Account": "Login"}</p>
        <p>{state == 'Sign Up' ? "Sign up": "log in "}to book Appointement</p>
        ...
        <button className='bg-indigo-500 text-white w-full py-2 rounded-md text-base'>{state == 'Sign Up' ? "Create Account": "Login"}</button>
          {
            state === 'Sign Up'
            ? <p className=''>Already have an account?<span onClick={()=> setstate('Login')} className='text-indigo-400 underline cursor-pointer'>Login here</span></p>
            : <p>Create an new account? <span onClick={()=> setstate('Sign Up')} className='text-indigo-400 underline cursor-pointer'>Click Here</span> </p>
          }
      </div>
