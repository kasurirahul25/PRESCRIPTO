...
const Appointment = () => {
  ...
  const {doctors,currencySymbol} = useContext(AppContect)
  ...
  const getAvailableSlots = async ()=>{
    ...
    for(let i=0; i <7 ;i++){
      ...
      endTime.setHours(24,0,0,0) // here we need to change it
        ...
        <div className='flex-1 border border-gray-400 rounded-lg p-8 py-7 bg-white mx-2 sm:mx-0 mt-[-80px] sm:mt-0'>
            <p className='flex items-center gap-2 text-2xl font-medium text-gray-900'>
              ...
              <div className='flex items-center gap-2 text-sm mt-1 text-gray-600'>
                ...
                <button className='py-0.5 px-2 border text-xs rounded-full text-gray-900'>{docInfo.experience}</button>
              </div>

              {/* display doctor about  */}
              <div >
                <p className='flex items-center gap-1 text-sm font-medium mt-3 text-gray-800 '>About <img src={assets.info_icon} alt="" /></p>
                <p className='text-sm text-gray-500 max-w-[700px] mt-1 '>{docInfo.about}</p>
              </div>
              <p className='text-gray-500 font-medium mt-4'>
                <span className='text-gray-600'>Appointment fee: {currencySymbol}{docInfo.fees}</span></p>
        </div>
      </div>
